...
public class ZInstruction {
	...
	case 0xC0: /* variable form */
		...
		/*
		 * note that variable-form je can have more than two arguments,
		 * though it is a 2OP
		 */
		opnum = opcode & 0x1F;
	    }
	...
	if (isbranch()) {
		...
		/*
		 * that is, if result is 0 and branchtype is false, or result is
		 * nonzero and branchtype is true
		 */
		switch (branchoffset) {
		case 0:
		    z_ret();
	...
	// System.err.println("insert_object " + operands[0] + " " +
	// operands[1]);
	...
	if ((new ZState(zm)).disk_save(save_pc))
	...
	if (restore_state.restore_from_disk()) {
	    ...
	    // branchoffset = (short)(((branchoffset&0x3F) << 8) |
	    // (((short)zm.get_code_byte())&0xFF));
	...
	if ((filesize > zm.memory_image.length)
	    ...
	    System.err.println("length\t" + filesize + "\t"
		    + zm.memory_image.length);
	...
	bufloc = 1;
	while ((tsize != 0) && ((ch = zm.get_input_byte(true)) != 13)
		&& (ch != 10)) {
	...
	if (operands[0] == 0) {
	    zm.zrandom.setSeed(System.currentTimeMillis()); /*
							     * back to random
							     * mode
							     */
	    return (short) 0;
	...
	cy = zm.window[LOWER_WINDOW].gety()
		+ zm.window[UPPER_WINDOW].getlines();
	cy -= lines;
	if (cy < 0)
	    cy = 0;

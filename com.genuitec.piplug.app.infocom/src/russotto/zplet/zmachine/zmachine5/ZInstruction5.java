...
public class ZInstruction5 extends ZInstruction {
	...
	if (operands[0] == 0) {
	    ...
	    /*
	     * a non-storing call to zero is pretty ridiculous, but hey, it
	     * could happen
	     */
	} else {
	...
	zm.locals = (short[]) tos;
	((ZMachine5) zm).argcount = (short) ((Integer) zm.zstack.pop())
		.intValue();
	// System.err.print("From ");
	// System.err.print(Integer.toString(zm.pc, 16));
	...
	bufloc = 2;
	/* to: prime input buffer here */
	ch = zm.get_input_byte(true);
	/* to: handle terminating character table here */
	...
	if (second == 0) {
	    ...
	    if (length > 0)
		System.arraycopy(zm.memory_image, first, zm.memory_image,
			second, length);
	    else {
		length = -length;
		for (i = 0; i < length; i++) {

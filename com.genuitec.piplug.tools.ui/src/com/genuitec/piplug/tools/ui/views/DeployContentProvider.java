...
public class DeployContentProvider implements IStructuredContentProvider,
	...
	BundleDescriptors localBundleDescriptors = PiPlugCore.getInstance()
		.getLocalBundleDescriptors();
	...
	for (BundleDescriptor next : descriptors) {
	    ...
	    if (null == bundle)
		continue;
	    SortedSet<PiPlugExtension> bundleExtensions = bundle
		    .getExtensions();
	    if (null != extensions)
	...
	if (null == display || display.isDisposed())
	    return;
	display.asyncExec(new DeployContentProvider.RefreshRunnable(viewer,
		view));
    }
